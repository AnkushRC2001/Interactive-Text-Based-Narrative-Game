...
def act2_main():
    ...
    pm("ACT II — THE CROSSING", style="bold")
    pm("(Beadon Street / Bagbazar Ghat — August 1946)", style="dim italic")
    pl("")

    # Scene 1 — Beadon Street Crossroads
    p("Scene 1 — Beadon Street Crossroads", style="bold")
    p("Rain soaks the cracked tramlines. The smell of burnt cloth and incense hangs thick. The sound of chanting drifts from the south — slogans, gunfire, screams.", style="dim")
    p("A police jeep lies overturned. A constable slumps beside it, his khaki shirt dark with blood. He raises a trembling hand.")
    p("'Help… the radio… please…'", style="italic")
    choice = present_choices(["Help the constable", "Ignore him and move on"])
    if choice == 1:
        state['morality'] += 1
        add_item(state, "whistle")
        p("You kneel, lift him. The radio hisses that lines are cut. He breathes 'Evacuation… through Bagbazar Ghat.' He dies. You take his whistle.")
        notify("Morality +1", style="success")
    else:
        state['morality'] -= 1
        p("You walk away. The man's plea dissolves in the rain.")
        notify("Morality -1", style="danger")

    # Scene 2 — Hedua Park Camp
    pl("\nScene 2 — Hedua Park Camp", style="bold")
    ...
    if c2 == 2:
        ...
        p("You tend the wounded; a nurse presses a silver locket into your palm. 'Maybe it will bring you luck.'")
        notify("Morality +1", style="success")
    else:
        p("You leave the camp to head for the river. The thought of Mitali sharpens your steps.")

    # Scene 3 — Bagbazar Ghat
    pl("\nScene 3 — Bagbazar Ghat", style="bold")
    ...
    if "locket" in chosen:
        ...
        p("The boatman takes the locket gently. 'Keep your seat, babu. We’ll cross before dawn.'")
        notify("Morality +1", style="success")
    elif "Threaten" in chosen:
        state['morality'] -= 2
        state['route'] = "river"
        p("You flash the knife and force him to row. The oar cuts the oily water.")
    ...
    if state['route'] == "river":
        pl("\nScene 4A — The River Crossing", style="bold")
        ...
        c = present_choices(["Shield the woman with your body", "Stay low and let fate decide"])
        if c == 1:
            p("You move to cover her...")
            # QTE
            if quick_time_event("DODGE BULLET", 2.0, lambda: None, lambda: None):
                state['morality'] += 1
                p("You pull her down just as wood splinters where she sat!", style="success")
                p("She whispers, 'Bless you, dada.'")
                notify("Morality +1", style="success")
            else:
                state['morality'] += 1
                state['health'] -= 1
                p("You shield her, but a bullet grazes your arm!", style="danger")
                p("She whispers, 'Bless you, dada.'")
        ...
        if state['morality'] >= 2:
            ...
            p("Across the bank, a child's voice calls: 'Baba!' Mitali runs to you as you stagger ashore.", style="success")
        else:
            p("You arrive alone. Refugees scatter. A child’s sandal lies in the mud — a painful sign you are not there yet.", style="dim")

    # Scene 5B — Ground Route
    else:
        pl("\nScene 5B — The Ground Route (Sovabazar & Nimtala)", style="bold")
        ...
        if c == 1:
            ...
            p("The old man whispers, 'Go by Nimtala… when you smell salt you're near.' He dies.")
            notify("Morality +1", style="success")
        else:
            state['morality'] -= 1
            state['health'] -= 1
            p("You move on. The old man's breath stops, and you carry the guilt.")
            notify("Health -1", style="danger")
            notify("Morality -1", style="danger")
        ...
        if c2 == 1:
            state['morality'] -= 1
            state['health'] -= 1
            p("You row alone, coughing and bleeding. The far bank takes you at dawn.")
            notify("Health -1", style="danger")
            notify("Morality -1", style="danger")
        else:
            state['morality'] += 1
            p("You join others. They chant for protection as hands pull each other in. The group slows risk through solidarity.")
            notify("Morality +1", style="success")
            
        if state['morality'] >= 2 and state.get('directions_to_nimtala', False):
            state['found_mitali'] = True
            p("A girl's voice rings out: 'Baba!' You find Mitali among the crowd and embrace her.", style="success")
        else:
            p("You come ashore alone. Only a blue ribbon drifts by your feet.", style="dim")

    save_state(state)
    show_status(state, after_act=2)
    pm("Act II complete. Run Act III when ready.", style="success")
